...
const validasi = (form) => {
  ...
  let copy = form.querySelector(".copy");
  if (name.value == "") {
  ...
  if (email.value == "") {
    ...
    giveError(email, "Please enter a valid email");
    valid = false;
  } else if (email.value.split("@")[0].length == 0) {
    giveError(email, "Please enter a valid email");
    valid = false;
  } else if (email.value.split("@")[1].length == 0) {
    giveError(email, "Please enter a valid email");
    valid = false;
  } else if (!email.value.split("@")[1].includes(".")) {
    giveError(email, "Please enter a valid email");
    valid = false;
  } else if (email.value.split("@")[1][0] == ".") {
    giveError(email, "Please enter a valid email");
    valid = false;
  }
  ...
  if (document.getElementById("copy").checked) {
    ...
    giveError(copy, "You must receive a copy");
    valid = false;
  }
